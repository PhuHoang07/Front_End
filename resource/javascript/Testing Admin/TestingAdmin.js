...
async function showModalAddExamSchedule() {
    ...
    const selectType = document.querySelector('#type-input');
    selectType.value = '';
...
async function addRowToTablesch() {
    ...
    const selectForm = document.querySelector('#form-input');
    const selectedFormIndex = selectForm.selectedIndex;
    const selectedFormOption = selectForm.options[selectedFormIndex];

    const selectType = document.querySelector('#type-input');
    const selectedTypeIndex = selectType.selectedIndex;
    const selectedTypeOption = selectType.options[selectedTypeIndex];
    ...
    const enteredValue = selectedFormOption.value;
    const selectedROptionText = selectedROption.text;
    const enteredtype = selectedTypeOption.value;
    const data = {
        body: {
            idt: idt,
    ...
    if (confirmation === true) {
        ...
        confirmButton.onclick = async function () {
            ...
            var modal = document.getElementById(
                'confirmationModalEditExamSchedule'
            );
...
function showStudentListToAdd() {
    const hiddenTable = document.getElementById('hiddenTable-3');
    const hiddenTable4 = document.getElementById(
        'hiddenTable-StudentListToAdd'
    );
    const textarea = document.getElementById('inputStu');
    textarea.value = '';
...
function showConfirmationModalRemoveStudentByButton(button) {
    var modal = document.getElementById(
        'confirmationModalRemoveStudentByButton'
    );
    modal.style.display = 'block';
    selectedButton = button;
...
async function confirmPublicTimeByButtonSelect(confirmation) {
    ...
    if (confirmation) {
        ...
        console.log(data);
        console.log(time);
        const res = await fetchAPIDataFilter(
            'https://swp-esms-api.azurewebsites.net/api/exams/time/public',
            'PATCH',
            data
        );
        renderExamTime();
...
async function confirmPrivateTimeByButtonSelect(confirmation) {
    ...
    if (confirmation) {
        ...
        console.log(time);
        const res = await fetchAPIDataFilter(
            'https://swp-esms-api.azurewebsites.net/api/exams/time/private',
            'PATCH',
            data
        );
        renderExamTime();
