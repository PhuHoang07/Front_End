...
function showConfirmationModalEdit(button) {
    const hiddenTable = document.getElementById('confirmationModal-2');
    hiddenTable.style.display = 'block';
...
async function confirmRemove(confirmation) {
    ...
    if (confirmation) {
        ...
        const res = await fetchAPIData(
            'https://swp-esms-api.azurewebsites.net/api/exams/delete-time',
            'POST',
            data
        );
        if (res.isSuccess == true) {
...
async function addRowToTable2() {
    ...
    const res = await fetchAPIData(
        'https://swp-esms-api.azurewebsites.net/api/exams/add-time',
        'POST',
        data
    );
    console.log(res);
